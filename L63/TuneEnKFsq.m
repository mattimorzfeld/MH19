function TuneEnKFsq(Ne,inflAll,SetUp,Gap)

load(strcat('./SetUps/SetUp_',num2str(SetUp),'_Gap_',num2str(Gap),'.mat'))

for kk = 1:length(inflAll)
    infl = inflAll(kk);
    [Xa,xAll,traceP] = mySqEnKF(Ne,infl,z,dt,dT,sqrtQ,H,R);
    ...
    Filename = strcat('./Results/sqEnKF_Results_SetUp_',num2str(SetUp),'_Gap_',num2str(Gap), ...
