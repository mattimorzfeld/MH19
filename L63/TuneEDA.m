function TuneEDA(Ne,inflAll,SetUp,Gap)

load(strcat('./SetUps/SetUp_',num2str(SetUp),'_Gap_',num2str(Gap),'.mat'))

for kk = 1:length(inflAll)
    infl = inflAll(kk);
    [Xa,xAll,traceP] = EDA(Ne,z,xo,sqrtB,infl,dt,dT,H,R);
    ...
    Filename = strcat('./Results/EDA_Results_SetUp_',num2str(SetUp),'_Gap_',num2str(Gap), ...
